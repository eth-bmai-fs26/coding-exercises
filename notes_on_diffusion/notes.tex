...
\[
  \bar{\alpha}_t \;=\; \alpha_1 \cdot \alpha_2 \cdots \alpha_t,
\]
that is, $\bar{\alpha}_t$ is simply the product of all the $\alpha$ values from the first step up to and including step $t$.
...
We also fix a source of \textbf{standard noise} $\varepsilon$: in one dimension a scalar, in $d$ dimensions a vector with independent standard bell-shaped coordinates, each with centre $0$ and spread $1$.
