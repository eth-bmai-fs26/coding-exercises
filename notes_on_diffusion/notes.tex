...
To find the \emph{expected} value of $x_{t-1}$ given $x_t$ (i.e.\ what we should aim for when reversing), we substitute the expression for $x_0$ from (A) and ignore the residual noise $\varepsilon'$ (which represents the irreducible randomness at step $t-1$ that we will add back at the end):
\[
...
The mean computed in (D) is the best deterministic guess for $x_{t-1}$ given $x_t$.
However, $x_{t-1}$ is not determined exactly by $x_t$: there is residual randomness corresponding to the fresh noise that was injected at step $t$.
In the DDPM framework, this residual is modelled as a normal cloud with spread $\sqrt{\beta_t}$.
Adding the corresponding noise term $\sqrt{\beta_t}\,z$, where $z$ is a freshly drawn standard noise vector, gives the full reverse sampling formula:
\[
x_{t-1}
